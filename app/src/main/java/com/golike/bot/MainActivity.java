package com.golike.bot;
  ...
  public class MainActivity extends AppCompatActivity {
      ...
      protected void onCreate(Bundle savedInstanceState) {
          ...
          updateLoadingText("Khoi dong bot service...");
          ...
          updateLoadingText("Dang ket noi server...");
          handler.postDelayed(this::tryLoadBot, 3000);
      }

      private void tryLoadBot() {
          if (pageLoaded) return;
          retryCount++;
          updateLoadingText("Dang ket noi... (" + retryCount + "/" + MAX_RETRY + ")");
          webView.loadUrl("http://localhost:" + BOT_PORT);
      }

      private void scheduleRetry() {
          if (pageLoaded || retryCount >= MAX_RETRY) {
              if (!pageLoaded) showOfflinePage();
              return;
          }
          handler.postDelayed(this::tryLoadBot, RETRY_MS);
      }

      private void showOfflinePage() {
          String html =
              "<html><body style='background:#07070d;color:#ccc;font-family:monospace;" +
              "display:flex;flex-direction:column;align-items:center;justify-content:center;" +
              "height:100vh;margin:0;padding:20px;box-sizing:border-box;text-align:center'>" +
              "<div style='font-size:60px'>🤖</div>" +
              "<h2 style='color:#f85149'>Khong ket duoc bot</h2>" +
              "<p>Bot service chua chay.<br>" +
              "Can cai <b style='color:#58a6ff'>Termux</b> va Python:<br>" +
              "<code style='color:#3fb950'>pkg install python</code></p>" +
              "<button onclick='Android.retry()' " +
              "style='margin-top:20px;padding:12px 28px;background:#238636;" +
      ...
      private void setupWebView() {
          ...
          webView.addJavascriptInterface(new RetryBridge(), "Android");
          ...
          webView.setWebViewClient(new WebViewClient() {
              ...
              public void onPageFinished(WebView v, String url) {
                  ...
                  if (url.contains("localhost:" + BOT_PORT)) {
                      pageLoaded = true;
      ...
      class RetryBridge {
          ...
          public void retry() {
              handler.post(() -> {
                  retryCount = 0;
                  pageLoaded = false;
                  updateLoadingText("Dang thu lai...");
                  handler.postDelayed(() -> tryLoadBot(), 1000);
                  if (splashView != null) {
